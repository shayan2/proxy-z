...
var (
	errUnrecognizedAddrType = errors.New("unknow addr type for parse udp packet.")
	udpClientSrcAddr        = &net.UDPAddr{IP: net.IPv4zero, Port: 0}
...
func (s *KcpBase) serveUDPConn(udpPacket []byte, reply func([]byte) error) error {
	// RSV  Reserved X'0000'
	...
	if targetAddrSpec.FQDN != "" {
		_, addr, err := DEFAULT_RESOLEVER.Resolve(nil, targetAddrSpec.FQDN)
		if err != nil {
